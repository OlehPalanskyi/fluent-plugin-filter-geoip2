    ...
    def download_database(download_url, md5_url, database_path, md5_path)
      ...
      unless current_md5 == fetched_md5 then
        ...
        begin
          ...
          log.info "MV Path: %s" % Dir.glob('./tmp/' + File.basename(download_path, ".tar.gz") + '_*/' + File.basename(download_path, ".tar.gz")  + '.mmdb')[0]
          FileUtils.mv(Dir.glob('./tmp/' + File.basename(download_path, ".tar.gz") + '_*/' + File.basename(download_path, ".tar.gz")  + '.mmdb')[0], tmp_database_path)
          FileUtils.rm_rf('./tmp')
